...
function OnboardingTimeline() {
  return (
    <ol className="relative m-0 list-none space-y-0 border-l border-border pl-6">
...
export function WhiteLabelProgramSection() {
  return (
    <>
      <a
        href="#wl-main"
        className="sr-only focus:not-sr-only focus:fixed focus:left-4 focus:top-4 focus:z-[100] focus:rounded-lg focus:bg-primary focus:px-4 focus:py-3 focus:text-sm focus:font-semibold focus:text-primary-foreground focus:shadow-lg focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 focus:ring-offset-background"
      >
      ...
      <main
        ...
        tabIndex={-1}
      >
        {/* Hero atmosphere — StructuredSvgBg defers / disables on small viewports and reduced motion */}
        <section className="relative overflow-hidden border-b border-border">
          ...
          <div className="relative mx-auto max-w-6xl px-4 pb-20 pt-8 sm:px-6 sm:pb-24 sm:pt-10 lg:px-8 lg:pb-28 lg:pt-12">
            <header className="flex flex-col gap-4 rounded-2xl border border-border/80 bg-card/75 p-4 shadow-sm backdrop-blur-md sm:flex-row sm:items-center sm:justify-between">
              <Link
                href="/"
                className="inline-flex min-h-11 items-center gap-2 text-sm font-medium text-muted-foreground transition-colors hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 focus-visible:ring-offset-background"
              >
                <Home className="h-4 w-4 shrink-0" aria-hidden />
                Statxeo home
              </Link>
              <nav className="flex flex-wrap items-center gap-2" aria-label="White-label partner actions">
                <Button variant="outline" size="lg" className="min-h-11 rounded-full border-border touch-manipulation" asChild>
                  <Link href="/white-labeler/apply" className="gap-2">
                    <Sparkles className="h-4 w-4" aria-hidden />
                    Apply
                  </Link>
                </Button>
                <Button variant="secondary" size="lg" className="min-h-11 rounded-full touch-manipulation" asChild>
                  <Link href="/white-labeler/login" className="gap-2">
                    <LogIn className="h-4 w-4" aria-hidden />
                    Sign in
                  </Link>
                </Button>
                <Button variant="neo" size="lg" className="min-h-11 rounded-full touch-manipulation" asChild>
                  <Link href="/white-labeler" className="gap-2">
                    Tenant portal
            ...
            <div className="mx-auto mt-14 max-w-4xl text-center lg:mt-20">
              <Badge
                variant="outline"
                className="rounded-full border-primary/35 bg-primary/10 px-4 py-1.5 text-xs font-medium text-primary"
              >
              ...
              <div className="mt-10 flex flex-col items-stretch justify-center gap-3 sm:flex-row sm:flex-wrap sm:items-center sm:justify-center">
                <Button variant="neo" size="xl" className="min-h-12 rounded-full touch-manipulation" asChild>
                  <Link href="/white-labeler/apply" className="gap-2">
                    Start with an application
                    <ArrowRight className="h-4 w-4" aria-hidden />
                  </Link>
                </Button>
                <Button variant="outline" size="xl" className="min-h-12 rounded-full border-border touch-manipulation" asChild>
                  <Link href="/white-labeler">Explore the tenant portal</Link>
                </Button>
              </div>
            </div>
            ...
            <ul className="mx-auto mt-4 flex max-w-3xl flex-wrap items-center justify-center gap-2 sm:gap-3">
              {audienceSegments.map((label) => (
                <li key={label}>
                  <span className="inline-flex min-h-9 items-center rounded-full border border-border bg-card/60 px-4 py-2 text-sm text-muted-foreground">
                    {label}
                  </span>
                </li>
              ))}
        ...
        {/* Proof row — three commitments, icon + text (not color-only) */}
        <section className="border-b border-border bg-secondary/25 px-4 py-12 sm:px-6 lg:px-8" aria-labelledby="wl-commitments-heading">
          <div className="mx-auto max-w-6xl">
            ...
            <div className="grid gap-4 md:grid-cols-3">
              {clientSurface.map((point) => (
                <div
                  key={point}
                  className="flex gap-4 rounded-2xl border border-border bg-card/80 p-5 shadow-sm transition-colors hover:border-primary/30"
                >
            ...
            <div className="grid auto-rows-fr gap-4 lg:grid-cols-4">
              {bentoBlocks.map((block) => (
                <MotionSection
                  key={block.key}
                  className={cn(
                    "flex flex-col justify-between rounded-3xl border border-border bg-card/90 p-6 shadow-sm transition-shadow hover:shadow-md",
                    block.className,
                  )}
                >
                  <div>
          ...
          <div className="mx-auto max-w-6xl">
            ...
            <div className="grid gap-6 lg:grid-cols-2">
              <Card className="overflow-hidden border-border bg-card shadow-lg">
                <CardHeader className="border-b border-border bg-secondary/40 pb-5">
                  <Badge variant="outline" className="w-fit rounded-full border-border text-muted-foreground">
                    Client-facing view
                  </Badge>
                  <CardTitle className="pt-2 text-2xl font-semibold tracking-tight sm:text-3xl">
                ...
                <CardContent className="space-y-4 pt-6">
                  <div className="overflow-hidden rounded-2xl border border-border bg-secondary/25">
                    <div className="flex items-center gap-2 border-b border-border bg-muted/30 px-3 py-2">
                      ...
                      <span className="ml-2 truncate rounded-md border border-border bg-background/80 px-2 py-0.5 font-mono text-[10px] text-muted-foreground">
                  ...
                  <div className="grid gap-3 sm:grid-cols-3">
                    {[
                      ...
                      <div key={label} className="rounded-xl border border-border bg-secondary/20 p-4 text-center sm:text-left">
              ...
              <Card className="border-border bg-card shadow-lg">
                <CardHeader className="border-b border-border bg-primary/8 pb-5">
                  <Badge variant="outline" className="w-fit rounded-full border-primary/35 text-primary">
                ...
                <CardContent className="space-y-3 pt-6">
                  {[
                    ...
                    <div key={label} className="rounded-2xl border border-border bg-secondary/20 p-5">
        ...
        <section className="px-4 py-16 sm:px-6 lg:px-8" aria-labelledby="wl-journey-heading">
          <div className="mx-auto max-w-6xl rounded-3xl border border-border bg-card/50 p-6 shadow-sm backdrop-blur-sm sm:p-10">
              ...
              {storyChapters.map((chapter, i) => (
                <MotionSection key={chapter.title} delay={i * 0.06}>
                  <Link
                    href={chapter.href}
                    className={cn(
                      "group flex h-full min-h-[260px] flex-col justify-between rounded-2xl border border-border bg-card p-6 shadow-sm transition-all duration-200",
        ...
        <section className="px-4 py-16 sm:px-6 lg:px-8" aria-labelledby="wl-onboard-heading">
          <div className="mx-auto grid max-w-6xl gap-10 lg:grid-cols-[1fr_1fr] lg:gap-12">
            ...
            <div className="space-y-8">
              <Card className="border-border bg-card shadow-lg">
                <CardHeader className="border-b border-border bg-primary/8">
                  <Badge variant="outline" className="w-fit rounded-full border-primary/35 text-primary">
                    Tenant controls
                  </Badge>
                  <CardTitle className="pt-2 text-xl font-semibold tracking-tight sm:text-2xl">
                ...
                <CardContent className="grid gap-3 pt-6 sm:grid-cols-2">
                  {operatingSystems.map((item) => (
                    <div
                      key={item.title}
                      className="rounded-2xl border border-border bg-secondary/15 p-5 transition-colors hover:border-primary/25"
                    >
              ...
              <div className="rounded-3xl border border-border bg-card/80 p-6 shadow-sm">
                ...
                <Accordion type="single" collapsible className="mt-4 w-full">
                  {faqItems.map((item, i) => (
                    <AccordionItem value={`faq-${i}`} key={item.q} className="border-border">
        ...
        {/* Final CTA */}
        <section className="border-t border-border px-4 py-20 sm:px-6 lg:px-8" aria-labelledby="wl-cta-heading">
          <div className="mx-auto max-w-4xl text-center">
            ...
            <div className="mt-10 flex flex-col items-stretch justify-center gap-3 sm:flex-row sm:flex-wrap sm:justify-center">
              ...
              <WhiteLabelerDemoPortalForm
                ...
                buttonClassName="rounded-full border-border"
              />
              <Button variant="outline" size="xl" className="min-h-12 rounded-full border-border touch-manipulation" asChild>
                <Link href="/white-labeler/login">Sign in to portal</Link>
              </Button>
            </div>
          </div>
        </section>

        {/* Mobile sticky CTA — safe area, single primary */}
        <div
          className="fixed inset-x-0 bottom-0 z-40 border-t border-border bg-background/95 p-3 pb-[max(0.75rem,env(safe-area-inset-bottom))] backdrop-blur-md md:hidden"
          role="region"
          aria-label="Quick apply"
        >
          <div className="mx-auto flex max-w-lg items-center gap-2">
            <Button variant="neo" size="lg" className="min-h-12 min-w-0 flex-1 rounded-full touch-manipulation" asChild>
              <Link href="/white-labeler/apply" className="gap-2">
                Apply
                <ArrowRight className="h-4 w-4 shrink-0" aria-hidden />
              </Link>
            ...
            <WhiteLabelerDemoPortalForm
              ...
              buttonClassName="min-h-12 shrink-0 rounded-full px-4 touch-manipulation"
            />
            <Button variant="secondary" size="lg" className="min-h-12 shrink-0 rounded-full px-4 touch-manipulation" asChild>
              <Link href="/white-labeler/login">Sign in</Link>
            </Button>
          </div>
        </div>
      </main>
