...
export interface DepthCardProps {
  ...
  /** Custom background content rendered behind the card overlay */
  ...
  /** Optional call-to-action text shown in the content overlay */
  ctaLabel?: string;
  ...
  /** Custom content to render inside the card (overrides default title and description layout) */
...
const DepthCard: React.FC<DepthCardProps> = ({
  ...
  const showExpandedContent = isHovered || shouldDisableEffects;
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Content Overlay */}
        {children ? (
          <div className={cn("absolute inset-0 z-30", contentClassName)}>
            {children}
          </div>
        ) : (
          <div
            className={cn(
              "absolute inset-0 z-30 flex flex-col justify-end bg-linear-to-t from-black/80 via-black/40 to-transparent p-6",
              contentClassName,
            )}
          >
            {title && (
            ...
            {ctaLabel && (
              <div
                className={cn(
                  "mt-4 inline-flex items-center gap-2 text-sm font-medium uppercase tracking-[0.18em] text-white/90 transition-all duration-500 delay-100",
                  showExpandedContent
                    ? "translate-y-0 opacity-100"
                    : "translate-y-4 opacity-0",
                )}
