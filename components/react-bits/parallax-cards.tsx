...
export interface ParallaxCardsProps {
  ...
  images: string[];
  /** Optional custom card body renderer */
  renderCard?: (index: number, imageUrl: string) => ReactNode;
  /** Optional accessible label generator for each card */
  getCardAriaLabel?: (index: number, imageUrl: string) => string;
      ...
      >
        {visibleImages.map((imageUrl, idx) => (
          <button
            ...
            type="button"
            aria-label={
              getCardAriaLabel?.(idx, imageUrl) ??
              `Focus preview card ${idx + 1}`
            }
            className={cn(
              "absolute rounded-lg overflow-hidden shadow-2xl",
              !renderCard && "bg-cover bg-center",
              "cursor-pointer",
            ...
            style={{
              ...
              backgroundImage: renderCard ? undefined : `url(${imageUrl})`,
              transformStyle: "preserve-3d",
              pointerEvents: "auto",
              userSelect: "none",
            }}
          >
            {renderCard?.(idx, imageUrl)}
          </button>
        ))}
      </div>
    </div>
