...
type GlobeConstructorType = new (element: HTMLElement) => GlobeInstance;

type GlobeColorValue = string | string[];
...
function normalizePalette(color: GlobeColorValue | undefined, fallback: string) {
  if (Array.isArray(color)) {
    const palette = color.filter(Boolean);
...
function cssColorToRgba(color: string, alpha: number) {
...
function createPaletteTexture(color: GlobeColorValue) {
  const palette = normalizePalette(color, "rgb(30, 30, 30)");
  const canvas = document.createElement("canvas");
  canvas.width = 512;
  canvas.height = 512;
  ...
  palette.forEach((entry, index) => {
    gradient.addColorStop(
      palette.length === 1 ? 1 : index / (palette.length - 1),
      entry,
    );
  });
  ...
  const radial = context.createRadialGradient(140, 150, 24, 256, 256, 340);
  radial.addColorStop(0, "rgba(255,255,255,0.18)");
  radial.addColorStop(0.4, "rgba(255,255,255,0.04)");
...
function createPalettePicker(palette: string[]) {
...
const Globe: React.FC<GlobeProps> = ({
  ...
  const globeConstructorRef = useRef<GlobeConstructorType | null>(null);
  ...
  useEffect(() => {
    ...
    const loadLibrary = async () => {
      try {
        ...
        globeConstructorRef.current = module.default as unknown as GlobeConstructorType;
  ...
  useEffect(() => {
    if (error || !containerRef.current || !isLibraryReady || !globeConstructorRef.current) return;
    ...
    const initGlobeDeferred = () => {
      ...
      landMapImage.onload = () => {
        ...
        const primaryPalette = normalizePalette(deferredPrimaryColor, "rgb(59, 130, 246)");
        const neutralPalette = normalizePalette(deferredNeutralColor, "rgb(156, 163, 175)");
        const atmospherePalette = normalizePalette(deferredAtmosphereColor, neutralPalette[0]);
        const pickPrimaryColor = createPalettePicker(primaryPalette);
        const pickNeutralColor = createPalettePicker(neutralPalette);

        const GlobeRuntime = globeConstructorRef.current;
        if (!GlobeRuntime) {
          isInitializingRef.current = false;
          return;
        }

        const world = (new GlobeRuntime(container) as unknown as GlobeInstance)
          .globeImageUrl(createPaletteTexture(deferredGlobeColor))
          .backgroundColor("rgba(0, 0, 0, 0)")
          .showAtmosphere(showAtmosphere)
          .atmosphereColor(atmospherePalette[0])
          .atmosphereAltitude(atmosphereAltitude)
          .width(containerWidth)
          .height(containerHeight)
          .pointsData(dots)
          .pointColor(() => pickNeutralColor())
          .pointRadius(pointSize)
          .pointResolution(pointResolution)
          .pointAltitude(0)
          .pointsMerge(true)
          .arcColor(() => pickPrimaryColor())
          .arcStroke(0.25)
          .arcDashInitialGap(1)
          .arcDashLength(2)
          .arcDashGap(2)
          ...
          .labelText(() => "")
          .labelColor(() => primaryPalette[0])
          .labelDotRadius(0.3)
          .labelAltitude(0.002)
          .labelsTransitionDuration(250)
          .ringColor(() => {
            const ringColor = pickPrimaryColor();
            return (t: number) => cssColorToRgba(ringColor, 1 - t);
          })
          .ringMaxRadius(2)
          .ringPropagationSpeed(2)
          .ringRepeatPeriod(0);

        const globeMat = world.globeMaterial() as {
          transparent: boolean;
          opacity: number;
          shininess?: number;
        };
        ...
        if (typeof globeMat.shininess === "number") {
